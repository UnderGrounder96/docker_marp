...
<!-- Docker logs + >2 examples of each docker "file"  -->
...
<!-- Basically Docker is an open-source platform for developing, shipping, and running applications, with the focus of allowing us to separate our application from infrastructure code. While allowing us to deliver software faster -->
...
<!-- The main thing I want you to take from this course is, docker has the ability to package and run application inside isolated environments or containers. These containers are lightweight, and have all requirements for running our applications. -->
...
<!-- Docker streamlines the development lifecycle by allowing developers to use local containers. And these containers are excellent for CI/CD integration. So much so, even if the team is not going to use docker in development these containers should be integral for CI. When I worked at Nokia, we used to have a docker trigger that would check for code standard for example -->
...
<!-- Grafana, redis, cassandra, jenkins, debian, centos, fedora, rabbitmq,
gradle, golang, nginx -->
...
<!-- An image is a template with instructions for creating a Docker container. Often, an image is based on another image. For those with programming skills, can think of images as classes and containers as instance of these classes or objects. -->
...
<!-- containers are defined by their images and additional configuration options during its creation or start. The one thing I want you to know is, unsaved changes made to containers disappear when the container is removed. -->
...
<!-- volumes are the preferred mechanism to persist container data. because they're managed by docker itself. With that being said, any questions so far? -->
...
<!-- Configure CI (build job) with this repo https://github.com/UnderGrounder96/factorial_cpp.git -->
